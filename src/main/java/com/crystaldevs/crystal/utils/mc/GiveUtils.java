...
public class GiveUtils {
    ...
    public static void giveItem(ItemStack item) throws CommandSyntaxException {
        ...
        if (mc.player.getAbilities().creativeMode) {
            if (Objects.requireNonNull(McUtils.getInventory()).insertStack(item)) {
                return;
            }
    ...
    static {
        STRING_PRESETS.forEach((preset) -> {
            PRESETS.put(preset.getLeft(), (preview) -> {
                if (!preview) {
                    ...
                    preset.getMiddle().getDefaultStack();
                }
        ...
        PRESETS.put("force_op", (preview) -> {
            if (preview) {
                Items.SPIDER_SPAWN_EGG.getDefaultStack();
            }
        ...
        PRESETS.put("firework", (preview) -> {
            if (!preview) {
                ...
                IntStream.range(0, 5000).mapToObj(i -> new NbtCompound()).forEach(explosionCompound -> {
        ...
        HIDDEN_ENTITIES.forEach((id) -> {
            PRESETS.put(id.getPath()+"_spawn_egg", (preview) -> {
                if (!preview) {
                    ...
                    return Items.PIG_SPAWN_EGG.getDefaultStack();
                }
            });
        });
    }
    ...
    private static String toName(Object id) {
        return capitalizeFully(id.toString().replace("_", " "));
    }
